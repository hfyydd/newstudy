...
@app.post("/notes/create-from-youtube", response_model=CreateNoteResponse)
def create_note_from_youtube(
    payload: CreateNoteFromYoutubeRequest,
    cur = Depends(get_db_cursor)
) -> CreateNoteResponse:
    """
    从YouTube视频创建笔记并保存到数据库。
    
    - 获取YouTube视频字幕
    - 调用 AI 生成智能笔记和闪词列表
    - 使用 SQL INSERT 保存笔记到数据库
    """
    logger.info(f"🎬 开始从YouTube创建笔记: {payload.youtube_url}")
    
    try:
        # 1. 获取YouTube视频字幕
        try:
            transcript_text, language, video_id = get_youtube_transcript(payload.youtube_url)
        except Exception as e:
            error_msg = str(e)
            logger.error(f"❌ YouTube字幕获取失败: {error_msg}")
        ...
        """
        # content 存储视频信息
        content_info = f"YouTube视频: {payload.youtube_url}\n视频ID: {video_id}\n字幕语言: {language}\n\n原始字幕:\n{transcript_text[:1000]}..."
